...
class VideoProcessor:
    ...
    def save_results(self):
        ...
        # Solo genera reporte de texto (sin JSON)
    ...
    def _generate_text_report(self, stats: Dict):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            success_pct = stats['successful_frames'] / stats['total_frames'] * 100
            f.write(f"Tasa de éxito global: {success_pct:.2f}%\n\n")
            ...
            for led_idx, led_stats in stats['leds'].items():
                ...
                if led_stats['mean_position']:
                    ...
                    std_dev = led_stats['std_deviation']
                    f.write(f"    Total (distancia euclidiana): {std_dev:.4f} píxeles\n")
                    ...
                    min_x, max_x = led_stats['min_x'], led_stats['max_x']
                    min_y, max_y = led_stats['min_y'], led_stats['max_y']
                    f.write(f"    Límites en X: [{min_x:.2f}, {max_x:.2f}]\n")
                    f.write(f"    Límites en Y: [{min_y:.2f}, {max_y:.2f}]\n")
                else:
...
def main():
    ...
    try:
        ...
        print(f"✓ Reporte guardado en: {args.output}reporte_deteccion.txt")
